...
func (s *SearchService) Do(ctx context.Context, cookies []*http.Cookie, headers http.Header) (*SearchResult, error) {
	// Check pre-conditions
	if err := s.Validate(); err != nil {
		return nil, err
	}
	log.Println("checked precondiotions")
	...
	log.Println("got url for request")
	...
	log.Println("request performed")
	// Return search results
	ret := new(SearchResult)
	if err := s.client.decoder.Decode(res.Body, ret); err != nil {
		return nil, err
	}
	log.Println("decoded result body")
	return ret, nil
}
