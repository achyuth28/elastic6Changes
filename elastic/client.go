...
func (c *Client) PerformRequest(ctx context.Context, opt PerformRequestOptions) (*Response, error) {
	...
	for {
		...
		log.Println("made a new reuquest with ", conn.URL()+pathWithParams)
		...
		log.Println("cookie", req.Header.Get("cookie"))

		log.Println("printing request ")
		requestDump, err := httputil.DumpRequest((*http.Request)(req), true)
		if err != nil {
		  fmt.Println(err)
		}
		...
		log.Println("executed client .Do()")
		if IsContextErr(err) {
		...
		log.Println("made the request")
		// log.Println("dumping response request")
		// responseDump, terr := httputil.DumpResponse(res, true)
		// if terr != nil {
		//   fmt.Println(terr)
		// }
